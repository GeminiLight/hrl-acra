...
class Counter(object):
    ...
    def __init__(self, node_attrs_setting=[], link_attrs_setting=[], **kwargs):
    ...
    def count_partial_solution(self, v_net, solution):
        # node revenue
    ...
    def calculate_sum_network_resource(self, network, node=True, link=True):
    ...
    def calculate_sum_node_resource(self, network):
        n = np.array(network.get_node_attrs_data(self.node_resource_attrs)).sum()
        return n

    def calculate_sum_link_resource(self, network):
    ...
    def calculate_v_net_link_cost(self, v_net, solution):
        r"""Calculate the deployment cost of current v_net according to `link paths`."""
        sum_link_cost = 0
        for v_link, p_links in solution['link_paths'].items():
            for p_link in p_links:
    ...
    def summary_records(self, records):
        ...
        summary_info['long_term_weighted_revenue'] = self.revenue_service_time_weight * summary_info['long_term_aver_time_revenue'] + self.revenue_start_price_weight * summary_info['long_term_aver_revenue']
